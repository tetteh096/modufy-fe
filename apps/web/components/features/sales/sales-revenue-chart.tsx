...
import type { SalesTrendDay } from "@/types/api";
...
export function SalesRevenueChart({ data, currency, className }: SalesRevenueChartProps) {
    ...
    <div className={cn("space-y-3", className)}>
          ...
          return (
            ...
            >
              ...
              <div
                ...
                style={{ height: barPx }}
                title={`${trendChartDayTitle(day.date)}: ${formatMoney(day.total, currency)}${day.count ? ` · ${day.count} sale${day.count !== 1 ? "s" : ""}` : ""}`}
              />
      ...
      {!hasAny && (
        <p className="text-xs text-muted-foreground text-center py-1">
          No sales in the last {data.length} days — record one to see the trend.
        </p>
      )}
    </div>
  );
