    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          ...
          {product.stock_qty != null ? ` · ${product.stock_qty} in stock` : ""}
        </p>
...
export function ProductPickerModal({
  ...
  description = "Search your product catalog with pagination — ideal for large lists.",
}: {
  open: boolean;
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ["product-picker", debouncedSearch, page, storefrontOnly],
    queryFn: () =>
      inventoryApi.list({
        type: "product",
        status: "active",
        search: debouncedSearch || undefined,
        storefront_visible: storefrontOnly ? true : undefined,
        page,
        limit: PAGE_SIZE,
      }),
    enabled: open,
  ...
  const items = storefrontOnly
    ? (data?.items ?? []).filter((p) => p.storefront_visible)
    : (data?.items ?? []);
  const total = storefrontOnly ? items.length : (data?.total ?? 0);
  const selectedItem = items.find((p) => p.id === draftId);
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
              ...
              <p className="mt-1 text-xs">
                {storefrontOnly
                  ? "Try another search or enable products on your storefront first."
                  : "Try another search or add products in Inventory."}
              </p>
            </div>
          ) : (
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {items.map((product) => (
                  <ProductPickCard
                    key={product.id}
                    product={product}
                    selected={draftId === product.id}
                    onSelect={() => setDraftId(product.id)}
                  />
                ))}
              </div>
            </div>
